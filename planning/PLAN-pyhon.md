# SignalForge — AI Trading Workstation
...
- **AI integration**: OpenAI-compatible LLM client with structured outputs for trade execution
...
When writing code to make calls to LLMs, use an OpenAI-compatible client (OpenAI, Ollama, Groq, custom). Structured Outputs should be used to interpret the results.
...
4. Calls the LLM via OpenAI-compatible endpoint, requesting structured output
5. Parses the complete structured JSON response
6. Auto-executes any trades or watchlist changes specified in the response
7. Stores the message and executed actions in `chat_messages`
8. Returns the complete JSON response to the frontend (no token-by-token streaming — LLM inference is fast enough that a loading indicator is sufficient)
...
When `LLM_MOCK=true`, the backend returns deterministic mock responses instead of calling external LLM providers. This enables:
...
The `db/` directory in the project root maps to `/app/db` in the container. The backend writes `signalforge.db` to this path.
