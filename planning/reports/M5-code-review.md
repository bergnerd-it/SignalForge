...
## Corrective implementation follow-up — 2026-09-16

This section records changes after the independent review above. The original findings remain as the historical review of its stated source identity; they should not be read as a description of the current dirty tree. TASK-17 was reopened. The current disposition, actual tests, browser observations, and remaining gaps are in `planning/reports/research-M5.md` and `planning/reports/walkthrough-M5.md`.

The corrective schema now lives in V13, leaving V12 content unchanged. The first V13 test run exposed a pooled SQLite connection with foreign keys left off; migration cleanup now restores the pragma in autocommit mode and checks referential integrity before commit. At that stage, the backend suite passed 195 tests, while populated V12 paper-row preservation still needed a direct upgrade fixture.

Subsequent verification provided that fixture. An archived HEAD/V12 backend created a populated disposable portfolio, proposal, item, adoption, intents, receivable, valuation, and execution result. Current V13 preserved the original rows and V12 recorded checksum, passed foreign-key and immutability checks, and repeated startup at version 13. The first upgrade attempt exposed LF/CRLF checksum incompatibility; the runner now accepts only those two exact V12 line-ending forms, covered by a new regression test. The approved native AUTO_PAPER toggle persisted a mode transition and the coordinator created an accepted pending intent. A repeated manual evaluation then exposed a raw UNIQUE error; the service now returns the same proposal, verified by integration test and HTTP retest. A configured OpenAI-compatible provider answered a synthetic portfolio question and resisted one bounded instruction to invent cash and execution. M5 remains PARTIAL because the full timing, readiness, S2/S3, disable/re-enable, holdout-failure, pagination, and final export checks are incomplete. At that stage, the backend suite passed 196/196 with one Gradle worker; two unbounded runs hit an inherited M4 replay `SQLITE_BUSY` contention failure.

A controlled-clock test also exposed that a superseded late-acceptance proposal prevented reevaluation while the same decision session remained latest completed. Cycle identity now includes the proposed future open; late acceptance creates no past intent, and reevaluation can schedule the next supported open. At that stage, the serial backend suite passed 197/197 tests.

The final V13 HTTP archive passed ZIP integrity and CSV inspection. It revealed 11 repeated same-state valuation rows from coordinator polling in the disposable portfolio. Processing now skips a mark when its revision, source, balances, observation and calculated metrics match the latest mark; the integration suite checks that a repeated process adds no valuation. The 197-test serial backend suite still passes. Historical immutable duplicate rows in the disposable fixture were preserved.

A new pure service test verifies that a failed holdout exposure write stops the research assistant before any model call. The inherited deterministic replay test was also narrowed to sequential completed runs; its previous overlap intermittently failed with SQLite writer contention, while separate concurrency tests remain. At that stage, `./gradlew clean test` passed 31 suites / 198 tests / 0 failures. This does not prove all concurrent backtest SQLite contention is resolved.

Another disposable SQLite regression enables and disables AUTO_PAPER after accepting a future order. It confirms the intent remains PENDING and all mode transitions persist. The final backend suite passes 31 suites / 199 tests / 0 failures. Native disable/re-enable and the remaining timing matrix are still open.

The Angular valuation view had treated `null` equity as zero in its chart and displayed cash as equity with a fabricated zero return/drawdown. It now omits unsupported chart points and shows `UNAVAILABLE` plus the missing-data reason. The final frontend suite passes 50/50 tests; the declared Node 24 production build passes at 626.13 kB with one existing CSS budget warning. Pagination controls remain open.

The current source fixes the demonstrated WAITING reprocessing, cost policy, transaction/provenance rollback, corporate-action availability and ledger-date entitlement, future-session inference, exact valuation (including a later same-session mark), owner-aware automatic coordinator, fabricated assistant facts, typed paper read views, deep-link change detection, and ZIP provenance. New integration tests verify a later immutable bar import, configured opening costs, delayed distribution after a sale, rollback/retry when execution provenance insertion fails, and validation of missing activation IDs. The full twelve-scenario M5 prompt matrix, populated M4 upgrade, all S2/S3 and AUTO_PAPER boundaries, assistant attack/holdout failures, and pagination controls remain open. The native walkthrough passed the listed user flows but the AUTO_PAPER toggle was blocked by automatic approval review. The verdict is therefore **PARTIAL — M5 is still open**, not a blanket PASS.

### Final corrective follow-up — 2026-09-16

The later approved verification supersedes several open statements in the chronological review above. A populated archived V12 paper database upgraded to V13 with original rows, checksum, foreign keys, immutability, and repeat startup intact. Native Chrome enabled AUTO_PAPER and observed its durable mode history and automatic accepted pending intent. The configured live provider returned grounded disposable-portfolio facts and resisted one bounded fabrication prompt. A provider-failure unit test now proves deterministic fallback without financial-table mutation, while the separate holdout test proves a failed exposure write prevents the model call.

The final source also resolves the audit pagination defect: typed previous/next controls retain total, limit, offset, and completion metadata for proposals, valuations, intents, receivables, actions, and adoptions. Controlled-clock readiness tests verify S2/S3 use the latest eligible completed month end and block incomplete per-listing warm-up. Final verification is `./gradlew clean test` at 202/202, Angular 52/52, and the declared Node 24.21.0 production build at 632.75 kB with the existing CSS budget warning. M5 remains **PARTIAL** because the full twelve-scenario matrix, complete S2/S3/corporate-action/AUTO_PAPER timing coverage, inherited M4 native form/holdout walkthrough, and Docker remain unverified; no demonstrated defect from the last review is being hidden as evidence-only work.
