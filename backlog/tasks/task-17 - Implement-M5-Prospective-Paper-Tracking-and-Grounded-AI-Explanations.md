...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Preserve and checkpoint complete M4 baseline with git tag m4-checkpoint and verified build/test toolchain
- [ ] #2 Follow-on migration(s) after V11 creating paper portfolios, proposals, execution intents, snapshot adoptions, mode history, and audit structures without mutating existing migrations
- [ ] #3 EUR PAPER portfolio creation, funding isolation, server Clock authority, snapshot binding, and current data readiness validation
- [ ] #4 Immutable proposal lifecycle (proposed, accepted, rejected, superseded, waiting-for-observation, executed, blocked, missed) with concurrency CAS checks, strictly future open scheduling, and observation resolution
- [ ] #5 Shared execution and exact accounting engine reuse for paper fills, costs, splits, dividends, receivables, and prospective valuations
- [ ] #6 Opt-in AUTO_PAPER mode with durable mode history, local coordinator, pre-open intent persistence, and downtime recovery
- [ ] #7 Grounded AI research assistant with typed owner-scoped read tools, fact cards, evidence references, and graceful offline fallback
- [ ] #8 Frontend paper portfolio UI at /research/portfolios/:id, proposal review, mode controls, prospective metrics, and audit export (ZIP/CSV)
- [ ] #9 Comprehensive tests (controlled clock, arithmetic walkthrough, concurrency barriers, recovery, upgrade) and research-M5.md closeout report with paper-tracking.md
<!-- AC:END -->

## Implementation Plan

<!-- SECTION:PLAN:BEGIN -->
M5 corrective implementation:
1. Repair intent recovery, scheduling, AUTO_PAPER, idempotency, per-portfolio serialization, and transaction boundaries.
2. Correct readiness/calendar provenance, corporate-action causality, configured execution costs, and exact valuations.
3. Complete typed grounded assistant queries and owner-scoped paper audit APIs.
4. Complete Angular deep-link/state views with strict types and stale-response handling.
5. Add focused production-path tests for corrected M5 scenarios and upgrade preservation.
6. Run formatting, backend/frontend suites, production build, disposable native walkthrough where available, and update M5 documentation with only observed evidence.
<!-- SECTION:PLAN:END -->
